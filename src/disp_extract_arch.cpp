...
namespace dxa
{
	...
	int gpr_idx(ZydisRegister reg)
	{
		ZydisRegister enc = ZydisRegisterGetLargestEnclosing(
		    PS == 8 ? ZYDIS_MACHINE_MODE_LONG_64 : ZYDIS_MACHINE_MODE_LEGACY_32,
		    reg);
		...
		{
			if (enc >= ZYDIS_REGISTER_EAX && enc <= ZYDIS_REGISTER_EDI)
				return static_cast<int>(enc - ZYDIS_REGISTER_EAX);
		}
		return -1;
	}
